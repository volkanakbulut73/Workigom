...
export function FindSharePage({ onNavigate, currentUserId }: FindSharePageProps) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // userType bilgisini userProfiles'dan al
      const userId = userData.email;
      const userProfiles = JSON.parse(localStorage.getItem('userProfiles') || '{}');
      const savedProfile = userProfiles[userId] || {};
      setUserType(savedProfile.userType || '');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const myActiveRequest = requests.find((r: FoodDonationRequest) => 
      r.userId === userId && ['waiting', 'supporter_matched', 'qr_uploaded', 'payment_confirmed'].includes(r.status)
    );
    ...
    if (myActiveRequest) {
      ...
      if (myActiveRequest.status === 'waiting') {
        setStep('waiting');
      } else if (myActiveRequest.status === 'supporter_matched') {
        setStep('matched');
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      const latestRequests = JSON.parse(localStorage.getItem('menuMarketShares') || '[]');
    ...
    const timer = setInterval(() => {
      ...
      if (newRemaining <= 0) {
        ...
        toast.error('QR kod süresi doldu', {
          description: 'Lütfen destekçiden yeni QR kod talep edin'
        });
      }
    }, 1000);
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      ...
      if (updated && updated.status !== currentRequest.status) {
        ...
        if (updated.status === 'supporter_matched') {
          setStep('matched');
          toast.success('Destekçi bulundu!', {
            description: `${updated.supporterName} desteğini onayladı`
          });
    ...
    if (step === 'matched' && currentRequest?.isFullSupport) {
      ...
      const timer = setTimeout(() => {
        ...
        toast.success('✨ Ödeme yapmanıza gerek yok!', {
          description: 'Destekçi QR kodu hazırlıyor'
        });
      }, 3000);
  ...
  const handleSubmitRequest = () => {
    ...
    localStorage.setItem('menuMarketShares', JSON.stringify(requests));

    console.log('✅ Yeni paylaşım talebi oluşturuldu:', newRequest);
    console.log('📦 Toplam talep sayısı:', requests.length);

    setCurrentRequest(newRequest);
    setStep('waiting');
    
    toast.success('✅ Paylaşım isteğiniz yayınlandı!', {
      description: 'Destekçiler talepinizi görecek'
    });
  };
  ...
  const handlePaymentDone = () => {
    ...
    if (currentRequest.isFullSupport) {
      ...
      notifications.unshift({
        id: Date.now().toString(),
        userId: currentRequest.supporterId,
        type: 'share_completed',
        title: '💛 Paylaşım Tamamlandı',
      ...
      toast.success('Afiyet olsun! 🎓', {
        description: 'Destekçiye teşekkür bildirimi gönderildi'
      });

      setTimeout(() => {
        onNavigate('menu-market-home');
      }, 2000);
    }
  };

  const handleThankSupporter = () => {
    if (!currentRequest) return;
    ...
    // Destekçiye teşekkür bildirimi ve Altın Kalp
    const notifications = JSON.parse(localStorage.getItem('notifications') || '[]');
    notifications.unshift({
      id: Date.now().toString(),
      userId: currentRequest.supporterId,
    ...
    // Destekçinin Altın Kalp sayısını artır
    const userProfiles = JSON.parse(localStorage.getItem('userProfiles') || '{}');
    if (!userProfiles[currentRequest.supporterId!]) {
      userProfiles[currentRequest.supporterId!] = {
        userId: currentRequest.supporterId,
        userName: currentRequest.supporterName,
        goldenHeartCount: 0,
        totalSharesGiven: 0,
        totalSharesReceived: 0
      };
    }
    userProfiles[currentRequest.supporterId!].goldenHeartCount += 1;
    userProfiles[currentRequest.supporterId!].totalSharesGiven += 1;
    localStorage.setItem('userProfiles', JSON.stringify(userProfiles));

    toast.success('❤️ Teşekkür gönderildi!', {
      description: 'Destekçi Altın Kalp kazandı'
    });
  ...
  const handleCancelRequest = () => {
    ...
    // Eğer destekçi varsa, destekçiye bildirim gönder
    if (currentRequest.supporterId) {
      const notifications = JSON.parse(localStorage.getItem('notifications') || '[]');
      notifications.unshift({
        id: Date.now().toString(),
        userId: currentRequest.supporterId,
        ...
        message: `${currentRequest.userName} paylaşım talebini iptal etti.`,
        timestamp: new Date().toISOString(),
        read: false,
        requestId: currentRequest.id
      });
        ...
        <div className="bg-gradient-to-br from-[#012840] to-[#0367A6] text-white p-4 lg:p-6 rounded-b-3xl lg:rounded-none">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-white">Paylaşım Bul</h1>
            </div>
            <p className="text-white/80 text-sm">Yemek için paylaşım talep et 🍽️</p>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-[#3F9BBF] mt-1">
                  Destekçiler bu isimle görecek
                </p>
              </div>

              <div>
                <Label htmlFor="user-type" className="text-[#012840]">
                  Kullanıcı Tipi
                </Label>
                <Input
                  id="user-type"
                  type="text"
                  value={userType === 'student' ? '👨‍🎓 Öğrenci' : userType === 'employee' ? '👔 Çalışan' : 'Belirtilmemiş'}
                  disabled
                  className="mt-1 bg-gray-50 cursor-not-allowed"
                />
                <p className="text-xs text-[#3F9BBF] mt-1">
                  {!userType && 'Profilinizden kullanıcı tipinizi seçebilirsiniz'}
                  {userType && 'Profilinizden ayarlanmış'}
                </p>
              </div>
              ...
              {menuAmount && parseFloat(menuAmount) > 0 && (
                <div className="p-4 bg-[#C9E2F2]/20 rounded-xl space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-emerald-700">Min. paylaşım (%20):</span>
  ...
  if (step === 'waiting' && currentRequest) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#C9E2F2]/30 to-white pb-20 lg:pb-6">
        <div className="bg-gradient-to-br from-[#012840] to-[#0367A6] text-white p-4 lg:p-6 rounded-b-3xl lg:rounded-none">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-white">Paylaşım Bekleniyor</h1>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 lg:px-6 mt-4">
          ...
          <Card className="p-8 border-0 shadow-lg text-center">
            ...
            <p className="text-[#0367A6] mb-6">
              Destekçiler talepinizi görüyor. Biri paylaşmak istediğinde bildirim alacaksınız.
            </p>
  ...
  // Destekçi Eşleşti - Ödeme Bekleniyor
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#C9E2F2]/30 to-white pb-20 lg:pb-6">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-500 text-white p-4 lg:p-6 rounded-b-3xl lg:rounded-none">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-white">Destekçi Bulundu! 💛</h1>
          </div>
        </div>
          ...
          <Card className="p-6 border-0 shadow-lg">
              ...
              {currentRequest.isFullSupport ? (
                <>
                  <h3 className="text-[#012840] mb-2">
                    {currentRequest.supporterName} yemeğine "Buda Benden" diyerek tamamına destek olmak istiyor 💛
                  </h3>
                ...
                <>
                  <h3 className="text-[#012840] mb-2">
                    {currentRequest.supporterName} yemeğine paylaşmak istiyor 💛
                  </h3>
                  <p className="text-[#0367A6]">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between py-3 px-4 bg-emerald-50 rounded-lg">
                <span className="text-sm text-emerald-700">Destekçinin paylaşımı:</span>
              ...
              <div className="p-4 bg-amber-50 rounded-xl border border-amber-200">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm text-amber-900 leading-relaxed">
                    Destekçi şu anda QR kod hazırlıyor. Kısa süre içinde QR kod ile ödeme yapabileceksiniz.
                  </p>
                </div>
              </div>
            ) : (
              <>
                <div className="p-4 bg-blue-50 rounded-xl border border-blue-200 mb-4">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-blue-900 leading-relaxed">
                      ...
                      <br />
                      Ödeme yaptıktan sonra destekçi QR kodu yükleyecek.
                    </p>
                  </div>
                </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      const requests2 = JSON.parse(localStorage.getItem('menuMarketShares') || '[]');
                      ...
                      localStorage.setItem('menuMarketShares', JSON.stringify(updatedRequests2));
                      
                      // Destekçiye ödeme tamamlandı bildirimi
                      ...
                      toast.success('✅ Ödeme başarılı!', {
                        description: 'Destekçi QR kodu yükleyecek'
                      });
                    }, 2000);
                  }}
                >
              ...
              {currentRequest.isFullSupport && (
                <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border-2 border-amber-200">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-amber-900 leading-relaxed">
                      <strong>{currentRequest.supporterName}</strong> yemek ücretinizin tamamını ödeyerek <strong>"Buda Benden Olsun"</strong> dedi 💛
                    </p>
                  </div>
                </div>
              )}
              ...
              <Card className="p-6 bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200 mb-6">
                <div className="mb-4">
                  ...
                  <h3 className="text-amber-900 mb-2">
                    {currentRequest.supporterName} tam destek verdi!
                  </h3>
                ...
                <Button
                  ...
                  onClick={handleThankSupporter}
                >
                  <Heart className="w-4 h-4 mr-2" />
                  {currentRequest.supporterName}'ye Paylaşımı İçin Teşekkür Et
                </Button>
              </Card>
            )}
